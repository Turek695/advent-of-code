...
const inputTable: string[][] = testInput4_1.map((el:string) => el.split(''));

const rows = inputTable.length;
const cols = inputTable[0].length;

const word: string[] = 'XMAS'.split('');
...
function xmasFound(row: number, col: number, word: string[], directions: number[][], minWords: number = 1): number {
    ...
    function findInDirection(rowDir: number, colDir: number): 0 | 1 {
        let rowCurrent: number = row;
        let colCurrent: number = col;
...
function masFound(rowCenter: number, colCenter: number, word: string[]): number {
    ...
    const isProperLetters:boolean = Object.values(diagonalLetters)
